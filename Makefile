...
# the three kernels, boot configuration, and the parts of the game's data
# that fheroes2 SHIPS ITSELF and is free to redistribute — its own .h2d asset
# files and the scenarios written for it.
#
# The Heroes of Might and Magic II data files are NOT here and are not ours
# to ship. See the README for what goes in fheroes2/data/ and fheroes2/maps/,
# and where it legitimately comes from.
CARD_DIR = build/sd-card
GAME_DIR = $(CARD_DIR)/fheroes2
card: kernels
	@rm -rf $(CARD_DIR)
	@mkdir -p $(GAME_DIR)/data $(GAME_DIR)/maps $(GAME_DIR)/files/data
	...
	@cp host/fheroes2.cfg $(GAME_DIR)/
	@cp fheroes2/files/data/*.h2d $(GAME_DIR)/files/data/
	@cp fheroes2/maps/*.fh2m $(GAME_DIR)/maps/
	@echo "  STAGED $(CARD_DIR)/"
	@echo "  The Raspberry Pi firmware files and the Heroes II data are not staged."
	@echo "  Put HEROES2.AGG and its companions in $(GAME_DIR)/data/,"
	@echo "  and the .MP2 scenarios in $(GAME_DIR)/maps/, before writing the card."
