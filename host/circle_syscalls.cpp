...
// owns the hardware.
//
// THE PROBLEM. fheroes2 reads its AGG archives, its maps, its configuration
// and its save games through the C library and through std::filesystem —
// open/read, opendir/readdir, stat, mkdir. Those reach newlib, and newlib's
// Circle glue drives FatFs and the SD card directly.
...
// the line and returns, and the hardware core prints it. The remaining
// descriptor-table calls are still marshalled with SDL2Circle_CallOn0 and
// run unchanged on core 0.
//
...
// They touch the same FatFs state, so they are marshalled rather than left
// to run on the wrong core.
